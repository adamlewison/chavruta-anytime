...
export default async function DashboardPage() {
  ...
  let upcomingSessions: Awaited<ReturnType<typeof getDashboardSessions>>["upcomingSessions"] = [];
  let nextSession: Awaited<ReturnType<typeof getDashboardSessions>>["nextSession"] = null;
  let activeChaburas: Awaited<ReturnType<typeof getDashboardChaburas>> = [];
  let myChavrutas: Array<{ userId: string; name: string | null; image: string | null }> = [];
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-6 space-y-8">
                ...
                <Card className="hover:border-accent/50 transition-colors h-full">
                  <CardContent className="p-4 space-y-2">
                    ...
                    <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                      ...
                      {DateTime.fromJSDate(occ.startsAt).toLocal().toFormat("EEE, MMM d")}
                    </div>
                    <div className="text-xs text-muted-foreground">
                      {DateTime.fromJSDate(occ.startsAt).toLocal().toFormat("h:mm a")}
                    </div>
                  </CardContent>
                </Card>
              </Link>
              ...
              >
                <Card className="hover:border-accent/50 transition-colors">
                  <CardContent className="p-4 space-y-1.5">
                    <p className="font-medium text-sm line-clamp-2">{ch.name ?? "Chabura"}</p>
                    <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
      ...
      {/* New user CTA */}
      {upcomingSessions.length === 0 && myChavrutas.length === 0 && activeChaburas.length === 0 && (
          ...
          <CardContent className="p-6 text-center space-y-4 relative overflow-hidden">
            ...
            <div className="relative space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                Every chavruta starts somewhere. Find a learning partner today.
              </p>
            </div>
